...
extension PlotData {
    ...
    mutating public  func axesScale() {//}-> PlotData {
        ...
        var sTics = noSecondary ? adjustAxis(&sMin, &sMax) : yTics
       
        if sTics.0 != 0 && !settings.independentTics && !noSecondary   { // makes secondary axis tics use y axis tics or vica versa
            if yTics.0 != 0  {
